...
export class PhysicsContainer extends Container {
    ...
    {        
        this.x += (this.speed.x * deltaSeconds) + this.acceleration.x * Math.pow(deltaSeconds, 2);
        this.y += 2 * this.speed.y * deltaSeconds + this.acceleration.y * Math.pow(deltaSeconds, 2);
